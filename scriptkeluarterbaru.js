...
let hasCheckedOut = false; // Cegah absen berulang
let faceMatcher; // Variabel global untuk FaceMatcher
let labels = []; // Simpan label pengguna dari server
...
video.addEventListener('play', () => {
    setInterval(async () => {
        if (!faceMatcher || hasCheckedOut) return; // Tunggu FaceMatcher siap, cegah spam absen keluar
        ...
        // Pastikan hanya absen keluar jika wajah dikenali
        if (nama !== "unknown") {
            hasCheckedOut = true; // Tandai absen keluar sudah dilakukan
            await autoAbsenKeluar(nama);
        }
    }, 1000); // Periksa setiap 1 detik
});

// Fungsi otomatis melakukan absen keluar
async function autoAbsenKeluar(nama) {
    ...
    document.getElementById('waktuAbsenMasukInput').value = "--"; // Kosongkan karena ini absen keluar
    document.getElementById('waktuAbsenKeluarInput').value = currentTimeString;

    // Kirim data ke server
    await sendAbsenDataToServer(nama, currentDate, "--", currentTimeString, canvas.toDataURL('image/jpeg'));

    // Tampilkan pesan sukses
    Swal.fire({
        icon: 'success',
        title: 'Absen Keluar Berhasil',
        text: 'Anda telah berhasil melakukan absen keluar.',
        confirmButtonText: 'Ok',
    });

    console.log('Absen keluar otomatis berhasil:', nama);
}
...
async function sendAbsenDataToServer(nama, tanggalAbsen, waktuAbsenMasuk, waktuAbsenKeluar, imageDataURL) {
    ...
    try {
        ...
        const response = await fetch('proses_absen_keluar.php', {
            method: 'POST',
            body: formData,
        });

        if (response.status === 200) {
            console.log('Data absen keluar berhasil disimpan.');
        } else {
            console.error('Terjadi kesalahan saat menyimpan data absen keluar.');
        }
    } catch (error) {
